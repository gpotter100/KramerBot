...
def apply_vandalay_total_scoring(df: pd.DataFrame) -> pd.DataFrame:
  ...
  Produces:
    ...
    - fantasy_points / ppr / half mapped from total
  """
  if df.empty:
    return df.assign(
      vandalay_points=0,
  ...
  # Map to canonical fantasy fields
  receptions = df.get("receptions", 0)
  df["fantasy_points"] = df["vandalay_total_points"]
  df["fantasy_points_ppr"] = df["fantasy_points"] + receptions
  df["fantasy_points_half"] = df["fantasy_points"] + 0.5 * receptions
...
def apply_standard_scoring(df: pd.DataFrame, int_penalty: int = -1) -> pd.DataFrame:
  ...
  # PPR VARIANTS
  d["receptions"] = d.get("receptions", 0)
...
def apply_shen2000_scoring(df: pd.DataFrame) -> pd.DataFrame:
  ...
  Produces:
    ...
    - fantasy_points / ppr / half mapped from SHEN base
  """
  d = df.copy()
