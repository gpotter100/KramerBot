...
=============================== */
let attributionBarChart = null;

function renderAttributionBar(player) {
  const canvas = document.getElementById("attribution-bar");
  if (!canvas) return;

  const ctx = canvas.getContext("2d");

  if (attributionBarChart) {
    attributionBarChart.destroy();
    attributionBarChart = null;
  }

  const safe = v => (typeof v === "number" ? v : 0);

  const passing = safe(player.comp_passing_yards) + safe(player.comp_passing_tds);
  const rushing = safe(player.comp_rushing_yards) + safe(player.comp_rushing_tds);
  const receiving = safe(player.comp_receiving_yards) + safe(player.comp_receiving_tds);
  const fumbles = safe(player.comp_fumbles_lost);

  attributionBarChart = new Chart(ctx, {
    type: "bar",
    data: {
      labels: ["Fantasy Attribution"],
      datasets: [
        {
          label: "Passing",
          data: [passing],
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: ctx => `${ctx.dataset.label}: ${fmt1(ctx.raw)} pts`
          }
        }
      },
      scales: {
  ...
  document.getElementById("attribution-bar-wrapper").classList.remove("hidden");
}
